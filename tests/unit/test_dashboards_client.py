"""Unit tests for DashboardsClient."""
...
class TestDashboardsClientInitialization:
    """Test DashboardsClient wiring on the main client."""
...
class TestDashboardsClientGetAll:
    """Test DashboardsClient.get_all()."""

    def test_get_all_no_params(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            {"dashboards": [], "page": 1, "total": 0}
        )

        result = client.dashboards.get_all()
    ...
    def test_get_all_with_all_params(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
    ...
    def test_get_all_with_space_id(self, client, mock_transport):
        ...
        client.dashboards.get_all(space_id="marketing", validate_spaces=False)

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/s/marketing/api/dashboards"


class TestDashboardsClientCreate:
    """Test DashboardsClient.create()."""

    def test_create_minimal(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope(), status=201
        )

        result = client.dashboards.create(title="My dashboard")
    ...
    def test_create_with_all_fields(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope(), status=201
        )
    ...
    def test_create_with_space_id(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope(), status=201
        )
    ...
    def test_create_requires_title(self, client):
        with pytest.raises(ValueError, match="title"):
            client.dashboards.create(title="")


class TestDashboardsClientGet:
    """Test DashboardsClient.get()."""

    def test_get_success(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope()
        )

        result = client.dashboards.get(id="dash-1")
    ...
    def test_get_url_encodes_id(self, client, mock_transport):
        ...
        client.dashboards.get(id="my dashboard/1")

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/api/dashboards/my%20dashboard%2F1"

    def test_get_with_space_id(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope()
        )

        client.dashboards.get(id="dash-1", space_id="team-a", validate_spaces=False)

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/s/team-a/api/dashboards/dash-1"

    def test_get_requires_id(self, client):
        with pytest.raises(ValueError, match="id"):
            client.dashboards.get(id="")
        ...
        mock_transport.perform_request.return_value = _make_response(
            {
                "statusCode": 404,
                "error": "Not Found",
                "message": "A dashboard with saved object ID missing was not found.",
            },
            status=404,
        )

        with pytest.raises(NotFoundError):
            client.dashboards.get(id="missing")


class TestDashboardsClientUpdate:
    """Test DashboardsClient.update() (PUT upsert)."""

    def test_update_minimal(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope(title="Renamed")
        )

        result = client.dashboards.update(id="dash-1", title="Renamed")
    ...
    def test_update_with_fields_and_space(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response(
            _dashboard_envelope(), status=201
        )
    ...
    def test_update_requires_id(self, client):
        with pytest.raises(ValueError, match="id"):
            client.dashboards.update(id="", title="x")

    def test_update_requires_title(self, client):
        with pytest.raises(ValueError, match="title"):
            client.dashboards.update(id="dash-1", title="")

    def test_update_rejects_access_control(self, client):
...
class TestDashboardsClientDelete:
    """Test DashboardsClient.delete()."""

    def test_delete_success(self, client, mock_transport):
        mock_transport.perform_request.return_value = _make_response({}, status=204)

        client.dashboards.delete(id="dash-1")
    ...
    def test_delete_with_space_id(self, client, mock_transport):
        ...
        client.dashboards.delete(id="dash-1", space_id="team-a", validate_spaces=False)

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/s/team-a/api/dashboards/dash-1"

    def test_delete_requires_id(self, client):
        with pytest.raises(ValueError, match="id"):
