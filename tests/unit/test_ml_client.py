...
def client(mock_transport):
    """Create a Kibana client backed by a mocked transport."""
    return Kibana(_transport=mock_transport)


def _mock_response(mock_transport, body, status=200):
    response = ObjectApiResponse(body=body, meta=Mock(status=status, headers={}))
    mock_transport.perform_request.return_value = response
    return response


class TestMlClientInitialization:
    """Test MlClient initialization and wiring."""

    def test_ml_property_returns_ml_client(self, client):
        """client.ml is an MlClient instance bound to the parent client."""
        assert isinstance(client.ml, MlClient)
...
class TestMlClientSync:
    """Test MlClient.sync()."""

    def test_sync_default(self, client, mock_transport):
    ...
    def test_sync_simulate_true(self, client, mock_transport):
    ...
    def test_sync_simulate_false(self, client, mock_transport):
        """simulate=False is encoded as 'false', not omitted."""
        _mock_response(mock_transport, {})

        client.ml.sync(simulate=False)

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/api/ml/saved_objects/sync?simulate=false"

    def test_sync_space_scoped(self, client, mock_transport):
        """space_id prefixes the path with /s/{space_id}."""
        _mock_response(mock_transport, {})

        client.ml.sync(simulate=True, space_id="marketing", validate_spaces=False)
...
class TestMlClientUpdateJobsSpaces:
    """Test MlClient.update_jobs_spaces()."""

    def test_update_jobs_spaces(self, client, mock_transport):
    ...
    def test_update_jobs_spaces_per_item_error(self, client, mock_transport):
    ...
    def test_update_jobs_spaces_space_scoped(self, client, mock_transport):
        """space_id prefixes the path with /s/{space_id}."""
        _mock_response(mock_transport, {})
    ...
    def test_update_jobs_spaces_bad_request(self, client, mock_transport):
...
class TestMlClientUpdateTrainedModelsSpaces:
    """Test MlClient.update_trained_models_spaces()."""

    def test_update_trained_models_spaces(self, client, mock_transport):
    ...
    def test_update_trained_models_spaces_space_scoped(self, client, mock_transport):
        """space_id prefixes the path with /s/{space_id}."""
        _mock_response(mock_transport, {})
...
class TestMlClientErrorHandling:
    """Test MlClient error mapping."""

    def test_sync_authentication_error(self, client, mock_transport):
    ...
    def test_sync_not_found_error(self, client, mock_transport):
