"""Unit tests for AsyncTaskManagerClient."""
...
class TestAsyncTaskManagerClientInitialization:
    """Test AsyncTaskManagerClient initialization."""

    @pytest.mark.asyncio
    async def test_task_manager_client_initialization(self, mock_async_transport):
        """Test that AsyncTaskManagerClient can be initialized with a parent client."""
        client = AsyncKibana(_transport=mock_async_transport)
        task_manager_client = AsyncTaskManagerClient(client)
        assert task_manager_client._client is client

    @pytest.mark.asyncio
    async def test_task_manager_property_returns_task_manager_client(
        self, mock_async_transport
    ):
        """Test that client.task_manager returns an AsyncTaskManagerClient."""
        client = AsyncKibana(_transport=mock_async_transport)
        assert isinstance(client.task_manager, AsyncTaskManagerClient)

    @pytest.mark.asyncio
    async def test_task_manager_property_caching(self, mock_async_transport):
        """Test that the task_manager attribute returns the same instance."""
        client = AsyncKibana(_transport=mock_async_transport)
        assert client.task_manager is client.task_manager


class TestAsyncTaskManagerClientHealth:
    """Test AsyncTaskManagerClient.health() method."""

    @pytest.mark.asyncio
    async def test_health_success(self, mock_async_transport, mock_response):
        """Test successful health retrieval."""
        mock_async_transport.perform_request = AsyncMock(
            return_value=mock_response(body=SAMPLE_HEALTH_BODY, status=200)
        )

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.task_manager.health()
    ...
    @pytest.mark.asyncio
    async def test_health_warn_status(self, mock_async_transport, mock_response):
    ...
    @pytest.mark.asyncio
    async def test_health_takes_no_positional_arguments(self, mock_async_transport):
        """Test that health() rejects positional arguments."""
        client = AsyncKibana(_transport=mock_async_transport)

        with pytest.raises(TypeError):
            await client.task_manager.health("unexpected")  # type: ignore[call-arg]


class TestAsyncTaskManagerClientErrorHandling:
    """Test AsyncTaskManagerClient error handling."""

    @pytest.mark.asyncio
    async def test_health_not_found_error(self, mock_async_transport):
        ...
        with pytest.raises(NotFoundError):
            await client.task_manager.health()

    @pytest.mark.asyncio
    async def test_health_authorization_error(self, mock_async_transport):
