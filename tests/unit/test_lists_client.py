"""Unit tests for ListsClient."""

import pytest

from kibana._sync.client import Kibana
from kibana._sync.client.lists import (
    ListsClient,
...
class TestListsClientInitialization:
    """Test ListsClient initialization."""

    def test_lists_client_initialization(self, mock_transport):
        """Test that ListsClient can be initialized with a parent client."""
        client = Kibana(_transport=mock_transport)
        lists_client = ListsClient(client)
        assert lists_client._client is client

    def test_lists_property_returns_client(self, mock_transport):
        """Test that client.lists returns a ListsClient instance."""
        client = Kibana(_transport=mock_transport)
        assert isinstance(client.lists, ListsClient)

    def test_lists_property_caching(self, mock_transport):
        """Test that the lists property returns the same instance."""
        client = Kibana(_transport=mock_transport)
        assert client.lists is client.lists


class TestListsClientCreate:
    """Test ListsClient.create() method."""
    ...
    def test_create_with_all_options(self, mock_transport, mock_response):
...
class TestListsClientGet:
    """Test ListsClient.get() method."""
    ...
    def test_get_success(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.get(id="kbnpy-lists-unit")
    ...
    def test_get_encodes_id(self, mock_transport, mock_response):
        """Test that the list ID is URL-encoded in the query string."""
        mock_transport.perform_request.return_value = mock_response(body=_list_body())

        client = Kibana(_transport=mock_transport)
        client.lists.get(id="a list/id")

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/api/lists?id=a+list%2Fid"


class TestListsClientUpdate:
    """Test ListsClient.update() method."""

    def test_update_required_fields(self, mock_transport, mock_response):
    ...
    def test_update_with_version_fields(self, mock_transport, mock_response):
...
class TestListsClientPatch:
    """Test ListsClient.patch() method."""

    def test_patch_partial_body(self, mock_transport, mock_response):
        """Test that patch only sends the provided fields."""
        mock_transport.perform_request.return_value = mock_response(body=_list_body())

        client = Kibana(_transport=mock_transport)
        client.lists.patch(id="kbnpy-lists-unit", name="patched")
...
class TestListsClientDelete:
    """Test ListsClient.delete() method."""
    ...
    def test_delete_success(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.delete(id="kbnpy-lists-unit")
    ...
    def test_delete_with_reference_flags(self, mock_transport, mock_response):
...
class TestListsClientFind:
    """Test ListsClient.find() method."""
    ...
    def test_find_with_params(self, mock_transport, mock_response):
...
class TestListsClientIndex:
    """Test the value list data stream (index) methods."""

    def test_create_index(self, mock_transport, mock_response):
    ...
    def test_get_index_status(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.get_index_status()
    ...
    def test_delete_index(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.delete_index()
    ...
    def test_delete_index_in_space(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        client.lists.delete_index(space_id="sandbox", validate_spaces=False)

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/s/sandbox/api/lists/index"


class TestListsClientItems:
    ...
    def test_create_item(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.create_item(
    ...
    def test_create_item_with_id_and_meta(self, mock_transport, mock_response):
        """Test creating a list item with explicit id and meta."""
        mock_transport.perform_request.return_value = mock_response(body=_item_body())

        client = Kibana(_transport=mock_transport)
        client.lists.create_item(
            list_id="kbnpy-lists-unit",
            value="10.7.7.7",
            id="item-1",
            meta={"source": "unit"},
    ...
    def test_get_item_by_id(self, mock_transport, mock_response):
        """Test getting a list item by its ID."""
        mock_transport.perform_request.return_value = mock_response(body=_item_body())

        client = Kibana(_transport=mock_transport)
        client.lists.get_item(id="item-1")

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["method"] == "GET"
        assert call_kwargs["target"] == "/api/lists/items?id=item-1"

    def test_get_item_by_list_id_and_value(self, mock_transport, mock_response):
        """Test getting a list item by list_id and value."""
        mock_transport.perform_request.return_value = mock_response(body=_item_body())

        client = Kibana(_transport=mock_transport)
        client.lists.get_item(list_id="kbnpy-lists-unit", value="10.7.7.7")
    ...
    def test_get_item_requires_id_or_pair(self, mock_transport):
        """Test that get_item validates its selector parameters."""
        client = Kibana(_transport=mock_transport)

        with pytest.raises(ValueError):
            client.lists.get_item()
        with pytest.raises(ValueError):
            client.lists.get_item(list_id="kbnpy-lists-unit")
        with pytest.raises(ValueError):
            client.lists.get_item(value="10.7.7.7")
    ...
    def test_update_item(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.update_item(
    ...
    def test_patch_item(self, mock_transport, mock_response):
        """Test patching a list item."""
        mock_transport.perform_request.return_value = mock_response(body=_item_body())

        client = Kibana(_transport=mock_transport)
        client.lists.patch_item(id="item-1", value="10.7.7.9", refresh="true")
    ...
    def test_delete_item_by_id(self, mock_transport, mock_response):
        """Test deleting a list item by ID."""
        mock_transport.perform_request.return_value = mock_response(body=_item_body())

        client = Kibana(_transport=mock_transport)
        client.lists.delete_item(id="item-1")

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert call_kwargs["method"] == "DELETE"
        assert call_kwargs["target"] == "/api/lists/items?id=item-1"

    def test_delete_item_by_pair_with_refresh(self, mock_transport, mock_response):
        """Test deleting a list item by list_id/value with refresh."""
        mock_transport.perform_request.return_value = mock_response(body=_item_body())

        client = Kibana(_transport=mock_transport)
        client.lists.delete_item(
            list_id="kbnpy-lists-unit", value="10.7.7.7", refresh="wait_for"
        )
    ...
    def test_delete_item_requires_id_or_pair(self, mock_transport):
        ...
        with pytest.raises(ValueError):
            client.lists.delete_item()
        with pytest.raises(ValueError):
            client.lists.delete_item(list_id="kbnpy-lists-unit")
        mock_transport.perform_request.assert_not_called()

    def test_find_items(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.find_items(
            list_id="kbnpy-lists-unit",
            page=1,
            per_page=20,
            sort_field="value",
...
class TestListsClientExportItems:
    """Test ListsClient.export_items() method."""

    def test_export_items_request_shape(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.export_items(list_id="kbnpy-lists-unit")
    ...
    def test_export_items_registers_lenient_serializer(self, mock_response):
        ...
        mock_transport.perform_request.return_value = mock_response(body=["10.0.0.1"])

        client = Kibana(_transport=mock_transport)
        client.lists.export_items(list_id="kbnpy-lists-unit")

        assert isinstance(registry["application/ndjson"], _LenientNdjsonSerializer)


class TestListsClientImportItems:
    """Test ListsClient.import_items() method."""

    def test_import_items_multipart_body(self, mock_transport, mock_response):
    ...
    def test_import_items_new_list_with_type_and_filename(
        ...
        client = Kibana(_transport=mock_transport)
        client.lists.import_items(file=b"10.5.5.5\n", type="ip", filename="bad-ips.txt")
    ...
    def test_import_items_accepts_str_payload(self, mock_transport, mock_response):
        """Test that a str payload is passed through as UTF-8 bytes."""
        mock_transport.perform_request.return_value = mock_response(body=_list_body())

        client = Kibana(_transport=mock_transport)
        client.lists.import_items(file="10.5.5.5\n10.5.5.6\n", list_id="x")

        call_kwargs = mock_transport.perform_request.call_args[1]
        assert b"10.5.5.5\n10.5.5.6\n" in call_kwargs["body"]

    def test_import_items_requires_file(self, mock_transport):
        """Test that an empty file payload raises ValueError."""
        client = Kibana(_transport=mock_transport)

        with pytest.raises(ValueError):
            client.lists.import_items(file=b"", list_id="x")
        with pytest.raises(ValueError):
            client.lists.import_items(file=[], list_id="x")
        mock_transport.perform_request.assert_not_called()
...
class TestListsClientPrivileges:
    """Test ListsClient.get_privileges() method."""

    def test_get_privileges(self, mock_transport, mock_response):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.lists.get_privileges()
...
class TestListsClientErrorHandling:
    """Test ListsClient error handling."""
    ...
    def test_get_not_found_error(self, mock_transport, mock_response):
        ...
        with pytest.raises(NotFoundError, match="does not exist"):
            client.lists.get(id="kbnpy-lists-nope")

    def test_create_authorization_error(self, mock_transport, mock_response):
        ...
        with pytest.raises(AuthorizationException):
            client.lists.create(name="n", description="d", type="ip")
