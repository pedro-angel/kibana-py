"""Tests for AsyncNamespaceClient space support functionality."""
...
class TestAsyncNamespaceClientSpaceSupport:
    """Test space support functionality in AsyncNamespaceClient."""

    def test_init_with_default_space_id(self):
        """Test initialization with default space ID."""
        mock_client = Mock()
        client = AsyncNamespaceClient(mock_client, default_space_id="marketing")
    ...
    def test_init_with_validation_disabled(self):
        ...
        mock_client = Mock()
        client = AsyncNamespaceClient(mock_client, validate_spaces=False)
    ...
    @pytest.mark.asyncio
    async def test_validate_space_exists_success(self):
        ...
        await client._validate_space_exists("marketing")

        # Verify API was called
        mock_spaces_client.get.assert_called_once_with(id="marketing")

    @pytest.mark.asyncio
    async def test_validate_space_exists_not_found(self):
        ...
        with pytest.raises(SpaceNotFoundError) as exc_info:
            await client._validate_space_exists("nonexistent")

        assert exc_info.value.space_id == "nonexistent"
        assert "Space not found" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_validate_space_exists_404_error(self):
    ...
    @pytest.mark.asyncio
    async def test_validate_space_exists_other_error_reraises(self):
    ...
    @pytest.mark.asyncio
    async def test_validate_space_exists_no_spaces_client(self):
    ...
    @pytest.mark.asyncio
    async def test_space_validation_caching(self):
    ...
    @pytest.mark.asyncio
    async def test_space_validation_cache_negative_result(self):
    ...
    @patch("time.time")
    @pytest.mark.asyncio
    async def test_space_validation_cache_expiry(self, mock_time):
...
class TestAsyncNamespaceClientBackwardCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_perform_request_unchanged(self):
        ...
        result = await client.perform_request(
            method="GET",
            path="/api/test",
            params={"param": "value"},
    ...
    @pytest.mark.asyncio
    async def test_perform_request_enhances_error_with_space_context(self):
    ...
    @pytest.mark.asyncio
    async def test_perform_request_no_enhancement_for_non_space_path(self):
        ...
        with pytest.raises(ApiError) as exc_info:
            await client.perform_request(
                method="GET", path="/api/actions/connector/123"
            )
