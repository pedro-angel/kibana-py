"""Unit tests for AsyncAttackDiscoveryClient."""
...
from kibana._async.client.attack_discovery import (
...
def _mock_ok(mock_async_transport, body):
...
class TestAsyncAttackDiscoveryClientInitialization:
    """Test AsyncAttackDiscoveryClient initialization and wiring."""

    @pytest.mark.asyncio
    async def test_initialization(self, mock_async_transport):
        """Test that AsyncAttackDiscoveryClient can be initialized with a parent."""
        client = AsyncKibana(_transport=mock_async_transport)
        ad_client = AsyncAttackDiscoveryClient(client)
        assert ad_client._client is client

    @pytest.mark.asyncio
    async def test_property_returns_client(self, mock_async_transport):
        """Test that client.attack_discovery returns an AsyncAttackDiscoveryClient."""
        client = AsyncKibana(_transport=mock_async_transport)
        assert isinstance(client.attack_discovery, AsyncAttackDiscoveryClient)
...
class TestAsyncAttackDiscoveryFind:
    """Test AsyncAttackDiscoveryClient.find() method."""

    @pytest.mark.asyncio
    async def test_find_no_params(self, mock_async_transport):
        """Test find without parameters hits the bare _find route."""
        _mock_ok(mock_async_transport, _find_body())

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.find()
    ...
    @pytest.mark.asyncio
    async def test_find_param_encoding(self, mock_async_transport):
        """Test find encodes bools, lists and @timestamp correctly."""
        _mock_ok(mock_async_transport, _find_body())

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.find(
    ...
    @pytest.mark.asyncio
    async def test_find_id_filters(self, mock_async_transport):
    ...
    @pytest.mark.asyncio
    async def test_find_space_scoped(self, mock_async_transport):
        """Test find builds a space-scoped path with space_id."""
        _mock_ok(mock_async_transport, _find_body())

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.find(space_id="my-space", validate_spaces=False)

        call_kwargs = mock_async_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/s/my-space/api/attack_discovery/_find"


class TestAsyncAttackDiscoveryBulkUpdate:
    """Test AsyncAttackDiscoveryClient.bulk_update() method."""

    @pytest.mark.asyncio
    async def test_bulk_update_minimal(self, mock_async_transport):
        """Test bulk_update with only ids."""
        _mock_ok(mock_async_transport, {"data": []})

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.bulk_update(ids=["id-1", "id-2"])
    ...
    @pytest.mark.asyncio
    async def test_bulk_update_full_body(self, mock_async_transport):
        """Test bulk_update passes all optional update fields through."""
        _mock_ok(mock_async_transport, {"data": [{"id": "id-1"}]})

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.bulk_update(
            ids=["id-1"],
            enable_field_rendering=False,
...
class TestAsyncAttackDiscoveryGenerate:
    """Test AsyncAttackDiscoveryClient.generate() method."""

    @pytest.mark.asyncio
    async def test_generate_minimal(self, mock_async_transport):
        ...
        result = await client.attack_discovery.generate(
            alerts_index_pattern=".alerts-security.alerts-default",
    ...
    @pytest.mark.asyncio
    async def test_generate_full_body(self, mock_async_transport):
        """Test generate maps all optional snake_case args to camelCase."""
        _mock_ok(mock_async_transport, {"execution_uuid": "uuid-1"})

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.generate(
            alerts_index_pattern=".alerts-security.alerts-default",
            anonymization_fields=[],
            api_config={"actionTypeId": ".gen-ai", "connectorId": "conn-1"},
            size=100,
    ...
    @pytest.mark.asyncio
    async def test_generate_space_scoped(self, mock_async_transport):
...
class TestAsyncAttackDiscoveryGenerations:
    """Test the generations methods."""

    @pytest.mark.asyncio
    async def test_get_generations(self, mock_async_transport):
        """Test get_generations with query parameters."""
        _mock_ok(mock_async_transport, {"generations": [_generation_body()]})

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.get_generations(
    ...
    @pytest.mark.asyncio
    async def test_get_generations_no_params(self, mock_async_transport):
        """Test get_generations without parameters."""
        _mock_ok(mock_async_transport, {"generations": []})

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.get_generations()

        call_kwargs = mock_async_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/api/attack_discovery/generations"

    @pytest.mark.asyncio
    async def test_get_generation(self, mock_async_transport):
        """Test get_generation encodes the UUID path and query params."""
        _mock_ok(
            mock_async_transport, {"generation": _generation_body(status="started")}
        )

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.get_generation(
            execution_uuid="ede89de0-1a41-4fe5-9787-a7303e6836ac",
    ...
    @pytest.mark.asyncio
    async def test_get_generation_quotes_path(self, mock_async_transport):
        """Test get_generation URL-encodes special chars in the UUID."""
        _mock_ok(mock_async_transport, {"generation": _generation_body()})

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.get_generation(execution_uuid="a/b c")

        call_kwargs = mock_async_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/api/attack_discovery/generations/a%2Fb%20c"

    @pytest.mark.asyncio
    async def test_dismiss_generation(self, mock_async_transport):
        """Test dismiss_generation posts to the _dismiss route."""
        _mock_ok(mock_async_transport, _generation_body(status="dismissed"))

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.dismiss_generation(
            execution_uuid="ede89de0-1a41-4fe5-9787-a7303e6836ac"
        )
...
class TestAsyncAttackDiscoverySchedules:
    """Test the schedules methods."""

    @pytest.mark.asyncio
    async def test_create_schedule_minimal(self, mock_async_transport):
        """Test create_schedule with only the required properties."""
        _mock_ok(mock_async_transport, _schedule_body())
        ...
        result = await client.attack_discovery.create_schedule(
            name="kbnpy schedule",
            params=params,
            schedule={"interval": "24h"},
        )
    ...
    @pytest.mark.asyncio
    async def test_create_schedule_with_options(self, mock_async_transport):
        """Test create_schedule passes enabled and actions through."""
        _mock_ok(mock_async_transport, _schedule_body(enabled=True))

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.create_schedule(
            name="kbnpy schedule",
    ...
    @pytest.mark.asyncio
    async def test_get_schedule(self, mock_async_transport):
        """Test get_schedule builds the schedule path."""
        _mock_ok(mock_async_transport, _schedule_body())

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.get_schedule(
            id="b0ab787f-b77b-435f-a4bc-3fd7b82275cb"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_schedule(self, mock_async_transport):
        """Test update_schedule sends the full update body via PUT."""
        _mock_ok(mock_async_transport, _schedule_body(name="renamed"))
        ...
        result = await client.attack_discovery.update_schedule(
            id="b0ab787f-b77b-435f-a4bc-3fd7b82275cb",
            name="renamed",
            params=params,
    ...
    @pytest.mark.asyncio
    async def test_delete_schedule(self, mock_async_transport):
        """Test delete_schedule issues a DELETE on the schedule path."""
        _mock_ok(mock_async_transport, {"id": "b0ab787f-b77b-435f-a4bc-3fd7b82275cb"})

        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.delete_schedule(
            id="b0ab787f-b77b-435f-a4bc-3fd7b82275cb"
        )
    ...
    @pytest.mark.asyncio
    async def test_enable_schedule(self, mock_async_transport):
        """Test enable_schedule posts to the _enable route."""
        _mock_ok(mock_async_transport, {"id": "sched-1"})

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.enable_schedule(id="sched-1")
    ...
    @pytest.mark.asyncio
    async def test_disable_schedule(self, mock_async_transport):
        """Test disable_schedule posts to the _disable route."""
        _mock_ok(mock_async_transport, {"id": "sched-1"})

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.disable_schedule(id="sched-1")
    ...
    @pytest.mark.asyncio
    async def test_find_schedules(self, mock_async_transport):
        ...
        client = AsyncKibana(_transport=mock_async_transport)
        result = await client.attack_discovery.find_schedules(
    ...
    @pytest.mark.asyncio
    async def test_find_schedules_no_params(self, mock_async_transport):
        """Test find_schedules without parameters."""
        _mock_ok(
            mock_async_transport, {"page": 1, "per_page": 10, "total": 0, "data": []}
        )

        client = AsyncKibana(_transport=mock_async_transport)
        await client.attack_discovery.find_schedules()

        call_kwargs = mock_async_transport.perform_request.call_args[1]
        assert call_kwargs["target"] == "/api/attack_discovery/schedules/_find"

    @pytest.mark.asyncio
    async def test_schedule_space_scoped(self, mock_async_transport):
...
class TestAsyncAttackDiscoveryErrorHandling:
    """Test AsyncAttackDiscoveryClient error mapping."""

    @pytest.mark.asyncio
    async def test_get_generation_not_found(self, mock_async_transport):
        ...
        with pytest.raises(NotFoundError):
            await client.attack_discovery.get_generation(
                execution_uuid="2e13f386-46cf-4d65-9e2b-68609e132ba5"
            )

    @pytest.mark.asyncio
    async def test_get_schedule_not_found(self, mock_async_transport):
        ...
        client = AsyncKibana(_transport=mock_async_transport)
        with pytest.raises(NotFoundError):
            await client.attack_discovery.get_schedule(id="missing")

    @pytest.mark.asyncio
    async def test_generate_bad_request(self, mock_async_transport):
