...
def _mock_ok(mock_transport, body: dict) -> None:
...
class TestUptimeClientInitialization:
    """Test UptimeClient initialization."""

    def test_uptime_client_initialization(self, mock_transport):
        """Test that UptimeClient can be initialized with a parent client."""
        client = Kibana(_transport=mock_transport)
        uptime_client = UptimeClient(client)
        assert uptime_client._client is client

    def test_uptime_property_returns_uptime_client(self, mock_transport):
        """Test that client.uptime returns an UptimeClient instance."""
        client = Kibana(_transport=mock_transport)
        assert isinstance(client.uptime, UptimeClient)

    def test_uptime_property_caching(self, mock_transport):
        """Test that the uptime property returns the same instance."""
        client = Kibana(_transport=mock_transport)
        assert client.uptime is client.uptime


class TestUptimeClientGetSettings:
    """Test UptimeClient.get_settings() method."""

    def test_get_settings_success(self, mock_transport):
        """Test successful settings retrieval."""
        _mock_ok(mock_transport, _settings_body())

        client = Kibana(_transport=mock_transport)
        result = client.uptime.get_settings()
    ...
    def test_get_settings_space_scoped(self, mock_transport):
        """Test that space_id prefixes the path with /s/{space_id}."""
        _mock_ok(mock_transport, _settings_body())

        client = Kibana(_transport=mock_transport)
        client.uptime.get_settings(space_id="marketing", validate_spaces=False)
...
class TestUptimeClientUpdateSettings:
    """Test UptimeClient.update_settings() method."""

    def test_update_settings_all_fields(self, mock_transport):
        ...
        client = Kibana(_transport=mock_transport)
        result = client.uptime.update_settings(
    ...
    def test_update_settings_partial_body_only_provided_keys(self, mock_transport):
        """Test partial update: omitted args are excluded from the body."""
        _mock_ok(mock_transport, _settings_body())

        client = Kibana(_transport=mock_transport)
        client.uptime.update_settings(cert_age_threshold=365)
    ...
    def test_update_settings_space_scoped(self, mock_transport):
        """Test that space_id prefixes the path with /s/{space_id}."""
        _mock_ok(mock_transport, _settings_body())
...
class TestUptimeClientErrorHandling:
    """Test UptimeClient error handling."""

    def test_get_settings_not_found_error(self, mock_transport):
        ...
        with pytest.raises(NotFoundError):
            client.uptime.get_settings()

    def test_update_settings_authorization_error(self, mock_transport):
