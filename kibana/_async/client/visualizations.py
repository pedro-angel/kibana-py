...
class AsyncVisualizationsClient(AsyncNamespaceClient):
    ...
    async def get_all(
        ...
        *,
        ...
        path = self._build_space_path("/api/visualizations", space_id)
        params: dict[str, Any] = {}
    ...
    async def get(
        ...
        *,
        ...
        await self._maybe_validate_space(space_id, validate_spaces)
        path = self._build_space_path(f"/api/visualizations/{_quote(id)}", space_id)
        return await self.perform_request(
            "GET",
            path,
            headers={"accept": "application/json"},
