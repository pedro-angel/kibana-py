    ...
    async def server_version(self) -> str | None:
        ...
        Raises:
            ApiError: If ``/api/status`` rejects the request (bad credentials,
                ...
                :meth:`~kibana._async.client.utils.NamespaceClient._require_capability`.

        Example:
            >>> await client.server_version()
            '9.5.2'
            >>> from kibana import is_supported
            >>> is_supported(await client.server_version())
            True
        """
        cache = self._server_version_cache
        if not cache.resolved:
            body = (await self.perform_request("GET", "/api/status")).body
            value = None
