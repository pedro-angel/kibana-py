...
def _ndjson_bytes(file: bytes | str | list[dict[str, Any]]) -> bytes:
    ...
    or a list of saved-object dicts, which are encoded one-JSON-per-line.
...
def _build_multipart_body(
    ...
    *,
    ...
    """Build a ``multipart/form-data`` body for the saved-objects import APIs.

    :param file: NDJSON file content for the ``file`` form field
    :param retries: Optional retry operations for the ``retries`` form field
        (used by the resolve-import-errors API)
    ...
    if retries is not None:
        parts.append(
            ...
            + json.dumps(retries).encode("utf-8")
            + b"\r\n"
        )
    parts.append(
        (
...
class SavedObjectsClient(NamespaceClient):
    ...
    def resolve(
        ...
        *,
        ...
        # Build request path using base class utility
        path = self._build_space_path(
            f"/api/saved_objects/resolve/{_quote(type)}/{_quote(id)}",
            space_id,
            validate_spaces=validate_space,
        )
