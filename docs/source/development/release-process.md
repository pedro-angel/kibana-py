...
- [ ] `make dod` reports **GO** — the tooling-certified verdict over every criterion in `dod.config`, including the integration matrix below. It preflights the environment first: a virtualenv created before a dependency entered `pyproject.toml` is refused, pointing at `make setup`, rather than surfacing later as an unrelated failure inside some leaf. The individual checks are listed here because a NO-GO tells you which one to re-run, not because passing them one by one substitutes for the gate.
