...
- **A scheme-less host string no longer silently routes traffic to
  ...
  lacking `://` now raises a `ValueError` naming the offending input and the
  expected form (`http://host:port` or `https://host:port`) before any
  transport is built. The fix rejects rather than guesses a default scheme,
  since silently assuming `http://` would surprise TLS deployments; valid
  `http://`/`https://` strings, dict hosts, and `cloud_id` are unaffected.
