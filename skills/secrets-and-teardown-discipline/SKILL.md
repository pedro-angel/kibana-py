...
Make secrets — and identifying details — structurally impossible to commit, grant the narrowest privilege that works, always tear down the ephemeral infrastructure you stood up and verify it reached zero, and provision only inside the lifecycle you own. Prevention is structural — not a reviewer remembering to look.
...
5. **Engineer one-command teardown, then verify zero — scoped to what this cycle provisioned.** On the ephemeral resources your own IaC stood up (and only those), set the flags that let a single destroy command actually complete (`force_destroy = true` on the buckets it created, `deletion_protection = false` on the databases it created) so nothing blocks teardown — never strip these rails from a resource you merely touched. After every ephemeral cycle, run the destroy *of that cycle's own state* and *then verify zero independently* — e.g. `terraform state list` returns empty AND a provider-side query (`gcloud asset search-all-resources`, `aws resourcegroupstaggingapi get-resources`, or a billing/cost report) shows no surviving resources. For a fully throwaway environment, delete the enclosing project/account afterward — one you created for this purpose, never one you did not; rule 6 wins on any doubt — to sweep up residue your IaC never tracked (staging buckets, default logs, leftover disks).
...
- A recorded end-to-end teardown showed `24 destroyed`, after which the project itself — a throwaway created for that build, owned end to end — was deleted "to remove the CI staging bucket and guarantee zero cost" — destroy, then delete the enclosing project you created to sweep residue the IaC never tracked. (The artifact captured the destroy count and the project-delete sweep; the explicit verify-zero check in between is the step this skill recommends adding, not one that recording demonstrates.)
