...
5. **Build the eval harness on the real path.** The harness must invoke the unchanged production *code path* — the same use-case function and the same decide gate — and feed fixtures through the same text pipeline, all in the harness's own environment: the code is production's, the substrate (data, accounts, side effects) is the test's. It measures the judge you ship, not a re-implementation of it.
