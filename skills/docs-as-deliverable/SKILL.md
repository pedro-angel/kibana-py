...
6. **Fence off superseded docs — supersede, don't delete.** Preserve historical/design artifacts for provenance and put a one-line banner at the top pointing to the current authoritative doc, so no reader mistakes a stale snapshot for truth. Deleting them loses the decision trail; keeping them unbannered leaves a trap — the banner is the only correct move.
