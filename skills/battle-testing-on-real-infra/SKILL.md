...
1. Re-assert "done" against live infra. Provision the real services (a real database, a real deployment target, a real provider account — instances stood up for this test, never the production estate), run the real agent or code path end-to-end through them, then tear those same services down. Treat provision → exercise → destroy-what-you-provisioned as one atomic act.
...
4. Record each live run as a machine-readable evidence artifact. Pin the exact command, the backends/endpoints used, the deployed revision or commit id, and a per-resource pass/fail. A captured file (JSON/YAML/log) — not prose, not "I checked it." And pin the **run, not the runner**: the machine's *properties* (architecture, OS, kernel, tool versions, timings) carry the evidence; its *identity* (hostname, username, home path, non-public IPs) carries only reconnaissance value — use a role name ("the x86_64 verification host"). Litmus test: would the artifact prove the same claim with the identifier replaced by a role? Then the identifier is leakage.
...
- Pinning the runner instead of the run: a private hostname, username, or internal IP in a committed evidence artifact — it proved nothing the machine's properties didn't, and it leaked. Found live: a cross-architecture verification artifact shipped a private machine's name to a public repo because specificity was rewarded and identity had no counterweight.
